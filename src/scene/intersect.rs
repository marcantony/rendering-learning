...
mod test {
    ...
    mod prepare_computations {
        ...
        fn the_under_point_is_below_the_surface() {
            let r = Ray::new(Point3d::new(0.0, 0.0, -5.0), Vec3d::new(0.0, 0.0, 1.0));
            let shape = Object::from(glass_sphere()).transform(
                InvertibleMatrix::try_from(transformation::translation(0.0, 0.0, 1.0)).unwrap(),
            );
            let i = Intersection::new(5.0, &shape);
            let xs = vec![i];
        ...
        mod refraction {
            ...
            fn get_objects() -> (Object<Sphere>, Object<Sphere>, Object<Sphere>) {
                ...
                (
                    Object::from(a_s).transform(a_transform),
                    Object::from(b_s).transform(b_transform),
                    Object::from(c_s).transform(c_transform),
                )
            }
            ...
            fn get_xs<'a>(
                ...
                b: &'a Object<Sphere>,
                c: &'a Object<Sphere>,
            ) -> Vec<Intersection<'a, Object<Sphere>>> {
                vec![
                    (2.0, a),
                    (2.75, b),
                    (3.25, c),
